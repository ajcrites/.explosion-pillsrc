...
" PHP syntax: comments {{{1
  ...
  syn region phpComment start="/\*" end="\*/" contained extend contains=@Spell,phpComment fold
  syn region phpComment start="/\*\*#@+" end="\*\*#@-" fold extend transparent
  set foldtext=CommentFoldText()
  function! CommentFoldText()
   let line = getline(v:foldstart)
   let endline = getline(v:foldend)
   let linetest = match(line, '#@+')
   let lineend = match(endline, '#@-')
   if linetest > 0 && lineend > 0
      let sub = substitute(line, '/\*\*#@+', getline(v:foldstart+1), 'g')
      return v:folddashes .sub . ' - <<DOC BLOCK>>'
   elseif linetest > 0
      let sub = substitute(line, '/\*\*#@+', getline(v:foldstart+1), 'g')
      return v:folddashes .sub . ' - >>OPEN BLOCK<<'
   endif
   let sub = substitute(line, '/\*\*', getline(v:foldstart+1), 'g')
   return v:folddashes .sub
  endfunction
  syn region phpComment start="//" end="$" contains=NONE display
  if version >= 600
...
"    endif
  ...
  " EXCEPTIONS: try/catch { } {{{2
   ...
   syn keyword phpException finally contained nextgroup=phpFinallyRegion skipwhite skipnl
   syn region phpFinallyBlock matchgroup=phpBraceException start=/{/ end=/}/ keepend extend
         \ contained transparent
...
"   hi phpIdentifierSimply guifg=DarkGray ctermfg=Brown
    ...
    hi phpStructure  cterm=bold ctermfg=DarkRed
    HiLink phpOperator          PreProc    " => Statement(Yellow) / Operator(Red)
    HiLink phpIdentifier        Identifier  " => Identifier(Cyan)
    HiLink phpIdentifierErratic	phpIdentifier

    HiLink phpVarSelector       PreProc
...
"    endif
      ...
      HiLink phpSuperglobal phpSpecialFunction         " => Special (orange/red)
  ...
  HiLink phpCase          Conditional         " => Statement (Yellow / Brown)
